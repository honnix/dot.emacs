...
Specifically, Diff-Hl mode is enabled in all buffers where `diff-hl--global-turn-on' would do it.
...
Specifically, Diff-Hl-Amend mode is enabled in all buffers where `turn-on-diff-hl-amend-mode' would
do it.
...
Specifically, Diff-Hl-Show-Hunk-Mouse mode is enabled in all buffers where
`diff-hl-show-hunk-mouse-mode' would do it.

With prefix ARG, enable Global Diff-Hl-Show-Hunk-Mouse mode if ARG is positive; otherwise, disable
it.
