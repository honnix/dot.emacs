...
In addition to any hooks its parent mode `javascript-mode' might have run, this mode runs the hook
`json-mode-hook', as the final or penultimate step during initialization.

\\{json-mode-map}" t)
(autoload 'jsonc-mode "json-mode"
"Major mode for editing JSON files with comments.

In addition to any hooks its parent mode `json-mode' might have run, this mode runs the hook
`jsonc-mode-hook', as the final or penultimate step during initialization.
